...
- **Page background** — Appearance Default / Upload / Browse stock. Stock = curated Unsplash (verified 200) with category + keyword filter; optional Pexels API key in localStorage (`lunaclock_v7_pexels_key`) for live search. Apply uses `background-size: cover` + `no-repeat` + `center` via **inline longhands** (never leave `background` shorthand from gradient — it sets inline `repeat` and tiles). Upload = JPEG data URL in `lunaclock_v7_bg_upload`. Compact keys `pbm`/`pbs` for stock mode+id; dynamic Pexels picks store meta in `lunaclock_v7_bg_stock_meta`
- **Timezone flag emojis** — regional indicator flags (e.g. 🇧🇪) often render as `BE` on Windows; prefer pictograph emojis (Brussels = 🧇). Guarded by `tests/timezone.spec.js`
